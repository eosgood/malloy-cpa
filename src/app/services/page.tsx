  ...
  return (
    <div className="bg-white">
      <Hero
        ...
        subtitle="Comprehensive accounting and tax services tailored to meet the unique needs of individuals and small businesses."
      />
      ...
      <div className="max-w-6xl mx-auto py-16 px-6">
        <div className="space-y-16">
          {services.map((service, index) => (
            <div key={index} className="bg-sky-50 rounded-xl p-8 border border-sky-200">
              <div className="grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <div className="flex items-center mb-4">
                    {service.icon}
                    <h2 className="text-2xl font-bold text-slate-900 ml-4">{service.title}</h2>
                  </div>
                  ...
                  <ul className="space-y-2">
                    {service.features.map((feature, featureIndex) => (
                      <li key={featureIndex} className="flex items-start">
                        <svg
                          className="w-5 h-5 text-sky-700 mr-3 mt-0.5 flex-shrink-0"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
